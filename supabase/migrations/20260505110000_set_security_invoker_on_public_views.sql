...
create or replace view public.active_seller_cards with (security_invoker = true) as
select
  u.id,
  u.first_name,
  u.last_name,
...
create or replace view public.seller_public_profiles with (security_invoker = true) as
select
  u.id,
  u.first_name,
  u.last_name,
...
create or replace view public.seller_public_reviews with (security_invoker = true) as
select
  r.id,
  o.seller_id,
  r.rating,
...
create or replace view public.current_user_order_details with (security_invoker = true) as
select
  o.id,
  o.truffle_id,
  o.buyer_id,
  ...
  o.completed_at,
  o.cancelled_at,
  o.buyer_delivery_reminder_sent_at,
  payout.status as payout_status,
  payout.stripe_transfer_id as stripe_transfer_id,
  refund.status as refund_status,
  refund.stripe_refund_id as stripe_refund_id,
  t.truffle_type,
  t.quality,
  t.weight_grams,
